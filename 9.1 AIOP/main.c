...
void displaySignedWordAsBlink(uint8_t LED, uint8_t highByte, uint8_t lowByte, char scaleFactor){
    ...
    switch (LED){
        ...
        case 2: PORTC ^=  (1 << 6); break;
        case 3: PORTC ^=  (1 << 7); break;
    }
...
int main() {
    ...
    while(1) {
        MPUburstReadRAW();
        // displaySignedWordAsBlink(1, RxBuffer[0], RxBuffer[1],5);
        // displaySignedWordAsBlink(2, RxBuffer[2], RxBuffer[3],5);
        displaySignedWordAsBlink(3, RxBuffer[4], RxBuffer[5],5);
        wait(10);
    }
}